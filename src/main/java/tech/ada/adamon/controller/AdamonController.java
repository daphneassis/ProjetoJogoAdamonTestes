...
public class AdamonController {
    ...
    public ResponseEntity<List<Adamon>> recuperarTodosAdamons() {
        return new ResponseEntity(adamonService.recuperarTodosAdamons(), HttpStatus.OK);//200
    }

    @PostMapping
    public ResponseEntity criarNovoAdamon(@RequestBody Adamon adamon) {
        return new ResponseEntity(adamonService.criarNovoAdamon(adamon), HttpStatus.CREATED) ;// 201
    }
